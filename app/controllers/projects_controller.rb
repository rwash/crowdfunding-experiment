class ProjectsController < InheritedResources::Base
  ...
  def create_projects                  # <TODO CL> Refactor and Move into Model.
    ...
    @build_group.projects.each do |project|
      ...
      # @current_group.projects << Project.new(:user_id => @user.id, :group_id => @current_group.id, :name => project.name, :value => project.value, :popularity => project.popularity)     
    end
        
  #   if @preference.finished_round == false    
  #     @build_group.projects.each do |project|
  #       project.user_id = @user.id
  #       project.group_id = @current_group.id
  #       @current_group.projects << project
